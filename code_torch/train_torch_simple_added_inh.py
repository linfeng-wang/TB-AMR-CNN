#adding more data input on top of the working simple model. 
#predicting only isoniazid resistance (inh) from all sequences
#%%
...
import util
import model_torch_simple_added_inh
from torchmetrics import Accuracy

model_torch_simple_added_inh = reload(model_torch_simple_added_inh)
...
res_all_combined = res_all["ISONIAZID"].values.tolist()
...
model = model_torch_simple_added_inh.raw_seq_model().to(device) # model = nn.Sequential(nn.Linear(1, 1)).to(device)
...
print(model.state_dict())
#%%
torch.save(model.state_dict(), '/mnt/storageG1/lwang/TB-AMR-CNN/code_torch/pytorch_model-simple_inh')
...
pd.DataFrame(history).to_csv("/mnt/storageG1/lwang/TB-AMR-CNN/code_torch/training-history-simple_inh.csv",index=False)
...
fig.savefig("/mnt/storageG1/lwang/TB-AMR-CNN/code_torch/batch-training-loss-simple_inh.png")
...
fig.savefig("/mnt/storageG1/lwang/TB-AMR-CNN/code_torch/batch-training-accuracy-simple_inh.png")
